...
public class OllamaService implements AIService {
        ...
        while (retries < MAX_RETRIES) {
            try {
                ...
                if (retries < MAX_RETRIES) {
                    System.err.println("Retrying Ollama embedding request after error: " + e.getMessage() +
    ...
    public String generateSummary(String codeContent, String fileName) throws IOException {
        ...
        jsonRequest.addProperty("prompt", "Generate a concise summary (max 3 sentences) of this " +
                getLanguageFromFileName(fileName) + " file. Include the main classes, methods, and functionality:\n\n" + truncatedCode);
        ...
        while (retries < MAX_RETRIES) {
            try {
                ...
                retries++;
                if (retries < MAX_RETRIES) {
                    System.err.println("Retrying Ollama summary generation after error: " + e.getMessage() +
                            " (Attempt " + retries + " of " + MAX_RETRIES + ")");
                    try {
                        Thread.sleep(RETRY_DELAY_MS);
                    } catch (InterruptedException ie) {
    ...
    public String generateCodeContext(String query, List<CodeSearchResult> results) throws IOException {
        ...
        while (retries < MAX_RETRIES) {
            try {
                ...
                retries++;
                if (retries < MAX_RETRIES) {
                    System.err.println("Retrying Ollama context generation after error: " + e.getMessage() +
                            " (Attempt " + retries + " of " + MAX_RETRIES + ")");
                    try {
                        Thread.sleep(RETRY_DELAY_MS);
                    } catch (InterruptedException ie) {
    ...
    public String askQuestionWithHistory(String question, List<CodeSearchResult> relevantFiles, List<Map<String, Object>> chatHistory) throws IOException {
        ...
        while (retries < MAX_RETRIES) {
            try {
                ...
                retries++;
                if (retries < MAX_RETRIES) {
                    System.err.println("Retrying Ollama question answering after error: " + e.getMessage() +
                            " (Attempt " + retries + " of " + MAX_RETRIES + ")");
                    try {
                        Thread.sleep(RETRY_DELAY_MS);
                    } catch (InterruptedException ie) {
